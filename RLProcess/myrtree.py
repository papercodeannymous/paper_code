...
class RTree:
    """
    Python wrapper for C++ R-Tree implementation with Reinforcement Learning integration.
    
    This class provides a Python interface to a custom C++ R-Tree library that supports
    both traditional R-Tree operations and RL-enhanced node selection. The wrapper handles
    all C++ function calls, data type conversions, and memory management between Python
    and the compiled C++ library.
    
    Key Features:
    - Traditional R-Tree operations (insertion, query, splitting)
    - RL-enhanced node selection during insertion
    - Multiple insertion and split strategies
    - State representation for RL agent training
    - Performance comparison capabilities
    """
    def __init__(self, max_entry, min_entry_factor):
        """
        Initialize R-Tree with C++ library binding and configuration.
        
        This constructor:
        1. Loads the compiled C++ shared library
        2. Defines function signatures for Python-C++ interoperability
        3. Initializes the C++ R-Tree object
        4. Sets up strategy mappings and configuration parameters
        
        Args:
            max_entry (int): Maximum number of entries per node (fanout)
            min_entry_factor (float): Factor to calculate minimum entries per node 
                                    (min_entries = max_entry * min_entry_factor)
        
        Note:
            The C++ library 'rtree_cppout/mytree' must be compiled and available
            in the specified path before instantiation.
        """
