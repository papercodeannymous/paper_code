/*
 * C++ Header File: myrtree.h
 * 
 * Core C++ implementation of the GSAR-Tree
 * with reinforcement learning integration. This header defines the underlying
 * data structures and algorithms that power the Python wrapper class.
 * 
 * Key Components:
 * - Strategy enumerations for insertion and splitting heuristics
 * - Core data structures: Rectangle, RTreeNode, RTree
 * - Algorithm implementations for spatial indexing and RL integration
 * - C interface functions for Python interoperability
 * 
 * Technical Features:
 * - Spatial indexing with efficient geometric data organization
 * - Multiple R-Tree variants (Guttman, R*-tree) support
 * - Reinforcement learning state representation extraction
 * - Manual memory management for C++ objects
 * - Clean C interface for Python binding via ctypes
 * 
 * Integration Note:
 * This C++ implementation serves as the performance-critical backend,
 * while the Python wrapper provides the RL training interface and
